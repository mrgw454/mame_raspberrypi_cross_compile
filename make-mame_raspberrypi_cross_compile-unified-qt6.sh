...
echo "Using build environment: $ENV_NAME"
confirm
...
#############################################
TOOLCHAIN_GLOB="$PROJECT_DIR/build/x-tools/${ENV_NAME}/bin"
...
if [ "$FULL_BUILD" -eq 1 ]; then
    ...
    MAME_DSTR_SUFFIX="$ENV_SUFFIX" ./mame-cross-compile.sh -o download -r "$DEBIAN_RELEASE" -a arm64
    confirm
    MAME_DSTR_SUFFIX="$ENV_SUFFIX" ./mame-cross-compile.sh -o prepare -r "$DEBIAN_RELEASE" -a arm64
    confirm
fi
...
TOOLCHAIN_DIR="build/x-tools/${ENV_NAME}/bin"
...
SYSROOT="build/x-tools/${ENV_NAME}/aarch64-rpi4-linux-gnu/sysroot"
...
export PATH="$PROJECT_DIR/build/x-tools/${ENV_NAME}/bin:$PATH"
